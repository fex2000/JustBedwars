...
public class HypixelApi
{
    ...
    private static readonly Stopwatch _apiStopwatch = new();
    private static readonly SemaphoreSlim _errorDialogSemaphore = new(1, 1);
    private static readonly MemoryCache _playerCache = new("PlayerCache");
    private static readonly MemoryCache _leaderboardCache = new("LeaderboardCache");
    private static readonly MemoryCache _uuidCache = new("UuidCache");
    private static readonly MemoryCache _guildCache = new("GuildCache");
    ...
    public async Task<Guild?> GetGuildAsync(string query, string type)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(_apiKey)) url = $"https://api.hypixel.net/guild?key={_apiKey}&{queryParam}";
            ...
            DebugService.Instance.Log(
                $"[HypixelApi] Response: {response.Substring(0, Math.Min(response.Length, 100))}...");
            ...
            var guild = new Guild
            {
                ...
                ExpByGameType = guildObj["guildExpByGameType"]?.ToObject<Dictionary<string, long>>() ??
    ...
    public async Task GetNamesForGuildMembers(List<GuildMember> members, IProgress<double>? progress)
    {
        ...
        foreach (var member in members)
            ...
            else if (string.IsNullOrEmpty(member.Name)) member.Name = "-- Username Error --";
    }
    ...
    public async Task<Player?> GetPlayerStats(string username)
    {
        ...
        try
        {
            ...
            var useV2Api = string.IsNullOrEmpty(_apiKey);
            var url = useV2Api
            ...
            else
            {
                ...
                player = new Player
                {
                    ...
                    FKDR = Math.Round(
                        ((double?)json["player"]?["stats"]?["Bedwars"]?["final_kills_bedwars"] ?? 0) /
                        ((double?)json["player"]?["stats"]?["Bedwars"]?["final_deaths_bedwars"] ?? 1), 2),
                    WLR = Math.Round(
                        ((double?)json["player"]?["stats"]?["Bedwars"]?["wins_bedwars"] ?? 0) /
                        ((double?)json["player"]?["stats"]?["Bedwars"]?["losses_bedwars"] ?? 1), 2),
                    KDR = Math.Round(
                        ((double?)json["player"]?["stats"]?["Bedwars"]?["kills_bedwars"] ?? 0) /
                        ((double?)json["player"]?["stats"]?["Bedwars"]?["deaths_bedwars"] ?? 1), 2),
                    BBLR = Math.Round(
                        ((double?)json["player"]?["stats"]?["Bedwars"]?["beds_broken_bedwars"] ?? 0) /
                        ((double?)json["player"]?["stats"]?["Bedwars"]?["beds_lost_bedwars"] ?? 1), 2),
    ...
    {
        var cacheKey = $"{leaderboard}_{time}_uuids";
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(_apiKey)) url = $"https://api.hypixel.net/leaderboards?key={_apiKey}";
    ...
    public async Task GetNamesForLeaderboardEntries(List<LeaderboardEntry> entries, IProgress<double>? progress)
    {
        ...
        foreach (var entry in entries)
            ...
            else if (string.IsNullOrEmpty(entry.Name)) entry.Name = "-- Username Error --";
    }

    private async Task<Dictionary<string, string>> GetUsernamesFromUuids(List<string> uuids,
        IProgress<double>? progress)
    {
        ...
        if (uuidsToFetch.Count > 0)
        {
            ...
            var totalToFetch = uuidsToFetch.Count;
                    ...
                    for (var i = 0; i < 5; i++)
                        try
                        {
                            ...
                            request.Headers.UserAgent.ParseAdd(
                                $"JustBedwars /v{Assembly.GetExecutingAssembly().GetName().Version}");
                        ...
                        {
                            DebugService.Instance.Log(
                                $"[HypixelApi] Timeout getting username for uuid {uuid}. Retry {i + 1}/5");
                        ...
                        catch (Exception ex)
                        {
                            ...
                            DebugService.Instance.Log(
                                $"[HypixelApi] Couldn't get username for uuid {uuid}: {ex.Message}. Retry {i + 1}/5");
                            if (i < 2) await Task.Delay(1000);
                        }
    ...
    public async Task<string?> GetUuidFromUsername(string username)
    {
        ...
        for (var i = 0; i < 3; i++)
            ...
            {
                DebugService.Instance.Log(
                    $"[HypixelApi] Error getting UUID for {username}: {ex.Message}. Retry {i + 1}/3");
                await Task.Delay(1000); // Wait 1s before retrying
            }
    ...
    private double GetGuildLevel(double exp)
    {
        ...
        {
            if (exp < expNeeded[i]) return level + exp / expNeeded[i];
            exp -= expNeeded[i];
            level++;
        }
