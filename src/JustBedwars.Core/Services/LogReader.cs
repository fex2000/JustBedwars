...
public class LogReader
{
    ...
    {
        _logFilePath = logFilePath ?? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData),
    ...
    {
        if (key == EnableLogReaderLoggingSettingName) LoadLoggingSetting();
    }
    ...
    private async Task WatchLogFile(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                if (new FileInfo(_logFilePath).Length < _lastPosition) _lastPosition = 0;
                ...
                using (var fs = new FileStream(_logFilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                {
                    ...
                    {
                        string? line;
                        while ((line = await sr.ReadLineAsync()) != null) ParseLine(line);
    ...
    {
        if (_enableLogging) DebugService.Instance.Log($"[LogReader] Parsing line: {line}");
        // /who command
        if (line.Contains("ONLINE:"))
        {
            var cleanedLine = Regex.Replace(line, @"^.*\[CHAT\]\s*", "");
            var players = cleanedLine.Replace("ONLINE: ", "")
        ...
        match = Regex.Match(line,
            @"\[.*\] \[Client thread/INFO\]: \[CHAT\] .*\uFFFD.\[.*\]\s(\w+)\uFFFD.:\s.*|\[.*\] \[Client thread/INFO\]: \[CHAT\].* \uFFFD.(\w+)\uFFFD.:\s");
        if (match.Success)
        {
        ...
        if (match.Success) ClearList?.Invoke();
    }
}