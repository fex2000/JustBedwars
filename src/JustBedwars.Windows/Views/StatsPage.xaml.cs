...
public sealed partial class StatsPage : Page
{
    ...
    protected override void OnNavigatedTo(NavigationEventArgs e)
    {
        ...
        if (e.Parameter is string username) LoadPlayerStats(username);
    }

    private void LoadApiKey()
    {
        var apiKey = _settingsService.GetValue(ApiKeySettingName);
        if (apiKey != null) _hypixelApi.SetApiKey((string)apiKey);
    }
    ...
    public async void LoadPlayerStats(string username)
    {
        ...
        if (player != null)
        {
            ...
            {
                PlayerImage.Source =
                    new BitmapImage(new Uri($"https://skins.jbw.fexei.at/fullbody/{player.PlayerUUID}"));
    ...
    private async void UsernameAutoSuggestBox_TextChanged(AutoSuggestBox sender,
        ...
        if (args.Reason == AutoSuggestionBoxTextChangeReason.UserInput)
        {
            ...
            try
            {
                var response = await _httpClient.GetStringAsync(
                    $"http://jbw.fexei.at/api/justbedwars/v2/autocomplete?query={query}&limit=10&mode=name");
    ...
    private void UsernameAutoSuggestBox_SuggestionChosen(AutoSuggestBox sender,
    ...
    private void UsernameAutoSuggestBox_QuerySubmitted(AutoSuggestBox sender,
    ...
    private async void ScreenshotButton_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        {
            var encoderId = file.FileType == ".png" ? BitmapEncoder.PngEncoderId : BitmapEncoder.JpegEncoderId;
            var encoder = await BitmapEncoder.CreateAsync(encoderId, stream);

            var alphaMode = file.FileType == ".png" ? BitmapAlphaMode.Premultiplied : BitmapAlphaMode.Ignore;
    ...
    private async Task AnimateScreenshotCard(bool back)
    {
        ...
        if (!back)
        {
            ...
            scaleAnimation.InsertKeyFrame(0.3f, new Vector3(0.9f, 0.9f, 1),
                CompositionEasingFunction.CreateCircleEasingFunction(compositor, CompositionEasingFunctionMode.InOut));
            scaleAnimation.InsertKeyFrame(0.35f, new Vector3(0.9f, 0.9f, 1));
            scaleAnimation.InsertKeyFrame(1, new Vector3(0.4f, 0.4f, 1),
                CompositionEasingFunction.CreateCircleEasingFunction(compositor, CompositionEasingFunctionMode.In));
            ...
            translationAnimation.InsertKeyFrame(0.3f, new Vector3(0, -5, 0),
                CompositionEasingFunction.CreateCircleEasingFunction(compositor, CompositionEasingFunctionMode.InOut));
            translationAnimation.InsertKeyFrame(0.35f, new Vector3(0, -5, 0));
            translationAnimation.InsertKeyFrame(1, new Vector3(0, 500, 0),
                CompositionEasingFunction.CreateCircleEasingFunction(compositor, CompositionEasingFunctionMode.In));
    ...
    public async Task<SoftwareBitmapSource> CreateBlurredImageSourceAsync(BitmapImage bitmapImage,
        float blurAmount = 10.0f, float darkenAmount = 0.4f)
    {
        try
        {
            if (bitmapImage == null || bitmapImage.UriSource == null) return await CreateTransparentImageSourceAsync();
            ...
            if (bitmapImage.PixelWidth == 0)
            {
                ...
                var loadSuccess = await tcs.Task;
            ...
            using (var ds = renderTarget.CreateDrawingSession())
            {
                ...
                if (darkenAmount > 0.0f)
                {
                    ...
                    var alpha = (byte)(255 * darkenAmount);
                    var overlayColor = Color.FromArgb(alpha, 0, 0, 0);
