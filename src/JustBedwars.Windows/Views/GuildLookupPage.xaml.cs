...
public sealed partial class GuildLookupPage : Page
{
    ...
    private void AddToHistory(string query)
    {
        ...
        if (_history.Count > maxHistorySize) _history = _history.Take(maxHistorySize).ToList();
    ...
    private async void SearchAutoSuggestBox_TextChanged(AutoSuggestBox sender, AutoSuggestBoxTextChangedEventArgs args)
    {
        ...
        switch (selectedItem.Content?.ToString())
        {
            case "Member Name":
                endpoint =
                    $"http://185.194.216.210:3000/api/justbedwars/v2/autocomplete?query={query}&limit=10&mode=name";
                break;
            case "Guild Name":
                endpoint =
                    $"http://185.194.216.210:3000/api/justbedwars/v2/autocomplete?query={query}&limit=10&mode=guild";
                break;
        }
    ...
    private void SearchAutoSuggestBox_QuerySubmitted(AutoSuggestBox sender, AutoSuggestBoxQuerySubmittedEventArgs args)
    {
        ...
        Frame.Navigate(typeof(GuildLookupResultPage), new GuildLookupParameter { Query = query, Type = type },
    ...
    private void HistoryListView_ItemClick(object sender, ItemClickEventArgs e)
    {
        ...
        Frame.Navigate(typeof(GuildLookupResultPage), new GuildLookupParameter { Query = clickedItem, Type = type },
            new DrillInNavigationTransitionInfo());
    }
